...
const std::string SNAPSHOT_LOG_FILE_PREFIX = "page_switch_snapshot";
...
constexpr size_t MAX_SANDBOX_LOG_FILE_SIZE = 1024 * 1024; // 1MB
constexpr size_t SANDBOX_LOG_MMAP_SIZE = 64 * 1024; // 64KB
constexpr int MAX_LOG_SIZE = 1024;
constexpr int SUCCESS = 0;
constexpr int ERROR_INTERNAL = -1; // Internal error
constexpr int ERROR_DISABLED = -2; // Log is disabled
...
SandboxLogger::SandboxLogger()
{
    ...
    logFileManager_ = std::make_unique<LogFileManager>();
...
int SandboxLogger::WriteLog(const std::string& str)
{
    ...
    int ret = snprintf_s(prefix, sizeof(prefix), sizeof(prefix) - 1,
        "%04d-%02d-%02d %02d:%02d:%02d.%03d  %5d  %5d ",
        tm_now.tm_year + 1900, tm_now.tm_mon + 1, tm_now.tm_mday,
        tm_now.tm_hour, tm_now.tm_min, tm_now.tm_sec,
        static_cast<int>(ms.count()), getpid(), gettid());
    if (ret < 0) {
        return ERROR_INTERNAL;
    }
    std::string fullMsg = std::string(prefix) + str;
    if (fullMsg.length() > MAX_LOG_SIZE + strlen(prefix)) {
        fullMsg.resize(MAX_LOG_SIZE + strlen(prefix));
    }
    // Ensure log ends with newline for proper line separation
    if (fullMsg.back() != '\n') {
        fullMsg += '\n';
    }
    queue_->submit(std::bind(&SandboxLogger::WriteLogToBuffer, this, fullMsg));
    return SUCCESS;
}

void SandboxLogger::WriteLogToBuffer(std::string str)
{
